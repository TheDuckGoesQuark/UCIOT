...
Locator values could be assigned using GPS for example.

Agricultural sensor networks often consist of many sensor nodes for monitoring the environment and a few actuators for triggering sprinklers or covering delicate crops. Based on this data, sensors could control the actuators within their locators themselves, whilst reporting their readings to the sink for further analysis. By splitting sensors into geographically relevant zones, the arriving data can also be grouped by origin location to help with visualisation and analysis at the sink. 

Our ZBR uses a different routing strategy for routing within and between locators, referred to as the intrazone and interzone routing protocols. The interzone routing protocol is based on the reactive protocol AODV with added fields to try and account for energy usage in paths. The intrazone routing protocol is based on the proactive link-state protocol OLSR.
...
A reactive protocol was chosen for inter-zone communication in order to limit the amount of global network knowledge required for routing. Since data is being sent for analysis purposes, delay is not a problem, and AODV could provide multiple paths which would allow traffic to be balanced across zones.

A proactive protocol was chosen for intra-zone communication in order to provide quick reliable routing within zones. For example, if high wind speeds are suddenly detected or a temperature drop due to snow, quick shelter could be crucial, like the one described in \cite{rain}.
...
The intra-zone routing table is constructed and maintained over the course of three phases: neighbourhood discovery, topology flooding, and topology updating.

\subsection{Neighbourhood Discovery}

Neighbourhood discovery is where a node learns about its one hop neighbours. A node starts by broadcasting its ID to neighbouring nodes with a \emph{hello} message.

\subsection{Topology Flooding}

\subsection{Topology Updating}
...
\begin{enumerate}
	...
	\item The next hop ID to reach this destination (i.e the ID of a border node from a neighbouring locator)
	\item The cost of this path, based on the average $\lambda$ within the zones being crossed.
\end{enumerate}
...
The inter-zone routing table is constructed through communication between border nodes across locators and AODVs route discovery mechanism.

AODV is a reactive protocol which has three phases: discovery, maintenance, and recovery.
